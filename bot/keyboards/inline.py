...
def get_question_detail_inline_kb(problem_obj: Problem, user_id: int, is_liked: bool = False, is_closed: bool = False):
    ...
    buttons.append(types.InlineKeyboardButton(text=emoji.emojize("Автор :copyright:", use_aliases=True),
                                              callback_data=author_callback))
    buttons.append(
        types.InlineKeyboardButton(text=emoji.emojize("Пожаловаться :warning:", use_aliases=True),
                                   callback_data=report_callback))

    if problem_obj.type == 'question':
        buttons.append(types.InlineKeyboardButton(text=emoji.emojize("Ответы :speech_balloon:", use_aliases=True),
                                                  callback_data=resp_or_disc_callback))
    else:
        buttons.append(
            types.InlineKeyboardButton(text=emoji.emojize("Перейти к обсуждению :speech_balloon:", use_aliases=True),
...
def get_resp_or_disc_inline_kb(problem_obj: Problem, user_id: int):
    ...
    buttons = [
        types.InlineKeyboardButton(
            text=emoji.emojize("Ответы других пользователей :incoming_envelope:", use_aliases=True),
            callback_data=others_responses_callback),
        types.InlineKeyboardButton(text=emoji.emojize("Написать свой ответ :pencil:", use_aliases=True),
