...
*5.*\t\t/my\_questions - твои вопросы/обсуждения
...
электронной почты (его увижу только я)"""  # кнопка exit
...
REGISTRATION_EXIT_MESSAGE = """Кажется, вы не зарегистрированы. Для регистрации используйте /register"""
...
# кнопка start
ABOUT_MESSAGE = '''Я - бот студентов МехМата! Нахожу и объединяю людей по учебным вопросам. Создав аккаунт, 
задавай свои вопросы или создавай обсуждения, чтобы разобраться в проблеме, а я постараюсь найти заинтересованных 
людей. 

Бот может решать твои проблемы через вопросы и обсуждения. Создай вопрос, если нужен ответ на один конкретный 
вопрос. Получив ответ, который тебе помог, закрой вопрос. Создай обсуждение, если вопросов по одной теме много и 
они требуют дискуссию. Можешь создать обсуждение, если хочешь найти людей для совместной учебы. 

Список всех команд можно получить через /help'''
...
опыт объяснения. Для начала необходимо создать аккаунт с помощью /register"""
# кнопка register
...
NEW_QUESTION_MESSAGE = """Сформулируй заголовок вопроса. Будь конкретен"""  # кнопка exit
...
NEW_QUESTION_THEME_MESSAGE = """Выбери не более 3 тем для своего вопроса"""  # кнопка exit
...
NEW_QUESTION_END_MESSAGE = """Вопрос сохранён: /detail{id}"""
...
NEW_DISCUSSION_END_MESSAGE = """Обсуждение сохранено: /detail{id}"""
...
QUESTION_DETAIL_DISLIKED_ALERT = "Ты убрал лайк и больше не отслеживаешь этот вопрос"
